...
Ten games, one avatar, one inventory, one save file. No backend, no account, no network.
...
> Native modules used (SQLite, haptics, blur, image picker) run in a development
> build or Expo Go. `npx expo run:android` / `run:ios` produce local builds.

---

## What it is

PocketVerse is deliberately **not** a launcher with ten unrelated games behind it.
...
├─ game/[id].tsx              THE GAME HOST — one screen hosts all ten games
