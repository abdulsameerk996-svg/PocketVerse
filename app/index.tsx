    ...
    <View style={styles.root}>
      ...
      <View style={[styles.bottom, { paddingBottom: insets.bottom + spacing.sm }]}>
        ...
        <View style={styles.panel}>
          {tab === 'shop' && <ShopPanel state={state} onBuy={(id) => { if (useTycoon.getState().buyGenerator(id)) playCue('buy'); }} />}
          {tab === 'upgrades' && <UpgradesPanel state={state} onBuy={(id) => { if (useTycoon.getState().buyUpgrade(id)) playCue('upgrade'); }} />}
          {tab === 'stats' && (
            <StatsPanel
              state={state}
              onPrestige={() => setShowPrestige(true)}
              onClaimMilestone={(id) => { if (useTycoon.getState().claimMilestone(id)) playCue('milestone'); }}
            />
          )}
        </View>
      </View>
